...
const fallbackImages = ["https://images.unsplash.com/photo-1466442929976-97f336a657be?auto=format&fit=crop&w=600&q=80", "https://images.unsplash.com/photo-1482049016688-2d3e1b311543?auto=format&fit=crop&w=600&q=80", "https://images.unsplash.com/photo-1490645935967-10de6ba17061?auto=format&fit=crop&w=600&q=80", "https://images.unsplash.com/photo-1414235077428-338989a2e8c0?auto=format&fit=crop&w=600&q=80", "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?auto=format&fit=crop&w=600&q=80"];

// Collection of decorative overlay patterns
const overlayPatterns = ["https://www.transparenttextures.com/patterns/cubes.png", "https://www.transparenttextures.com/patterns/food.png", "https://www.transparenttextures.com/patterns/cream-paper.png", "https://www.transparenttextures.com/patterns/restaurant.png", "https://www.transparenttextures.com/patterns/rice-paper-2.png"];
const RecipeCard = ({
  recipe,
  featured = false
}: RecipeCardProps) => {
  ...
  return <Link to={`/recipe/${recipe.id}`}>
      <Card className={`recipe-card h-full ${featured ? 'md:flex' : ''} overflow-hidden transform transition-all duration-300 hover:shadow-lg ${isHovered ? 'scale-[1.02]' : ''}`} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        <div className={`${featured ? 'md:w-2/5' : 'w-full'} relative overflow-hidden`}>
          <img src={recipeImage} alt={recipe.title} className={`w-full object-cover transition-transform duration-500 ${isHovered ? 'scale-110' : 'scale-100'} ${featured ? 'md:h-full md:aspect-auto' : 'aspect-[4/3]'}`} />
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-80" style={{
          ...
          <div className="absolute top-2 right-2 flex gap-2">
            ...
            {recipe.creator && recipe.creator.badges && recipe.creator.badges.length > 0 && <HoverCard>
        ...
        <CardContent className={`p-4 ${featured ? 'md:w-3/5' : ''} flex flex-col h-full`}>
          <h3 className="font-playfair font-semibold text-lg mb-2 line-clamp-2 text-center">{recipe.title}</h3>
          ...
          <div className="flex flex-wrap gap-1 mb-3">
            {recipe.tags && recipe.tags.slice(0, 3).map((tag, index) => <Badge key={index} variant="outline" className="text-xs bg-muted hover:bg-muted">
                {tag}
              </Badge>)}
          </div>
          
          <div className="mt-auto flex items-center pt-2 border-t border-gray-100">
            {recipe.creator && <div className="flex items-center">
                <img src={recipe.creator.avatar} alt={recipe.creator.name} className="h-6 w-6 rounded-full mr-2 object-cover border border-gray-100" />
