      ...
      <main>
        ...
        <section className="bg-gradient-to-r from-purple-500 to-pink-500 text-white py-16 relative overflow-hidden">
          <div className="absolute inset-0 opacity-20">
            <img src="https://images.unsplash.com/photo-1466442929976-97f336a657be?auto=format&fit=crop&w=2000&q=80" alt="Food background" className="w-full h-full object-cover" />
          </div>
          ...
          <div className="container mx-auto px-4 text-center relative z-10">
            ...
            <div className="max-w-md mx-auto flex gap-2">
              <div className="relative flex-1">
                ...
                <Input type="text" placeholder="Search recipes..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 w-full bg-white/90 text-black border-0" />
              </div>
        ...
        <section className="container mx-auto px-4 py-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {Object.entries(categoryImages).map(([category, imageUrl]) => <div key={category} className="relative rounded-xl overflow-hidden h-32 group cursor-pointer transform transition-transform duration-300 hover:scale-105" onClick={() => setActiveTab(category)}>
                <img src={imageUrl} alt={category} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
          ...
          <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value={activeTab} className="mt-0">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredRecipes.length > 0 ? filteredRecipes.map(recipe => <RecipeCard key={recipe.id} recipe={recipe} />) : <div className="col-span-full text-center py-16">
